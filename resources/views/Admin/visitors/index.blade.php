...
@section('title') {{$setting->title}} | Visitors Models @endsection
@section('page_name') Visitors Models @endsection
@section('css')
        ...
        <div class="col-md-12 col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">{{$setting->title}} Visitors Models</h3>
                    <i class="text-gray">" Visitors Models With Prices "</i>
                    <div class="">
...
@section('ajaxCalls')
    <script>
        ...
        showData('{{route('visitors.index')}}', columns);
        deleteScript('{{route('visitors.delete')}}');

        // Get Edit View
        $(document).on('click', '.editBtn', function () {
            var id = $(this).data('id')
            var url = "{{route('visitors.edit',':id')}}";
        ...
        $(document).on('click', '.addBtn', function () {
            ...
            $('#editOrCreate').modal('show')
            setTimeout(function () {
                $('#modalContent').load('{{route('visitors.create')}}')
            }, 250)
        });
